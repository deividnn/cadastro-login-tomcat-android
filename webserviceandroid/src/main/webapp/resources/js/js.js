...
function javascript() {
    ...
    $(".dinheiro").maskMoney({thousands: '', decimal: ',', symbolStay: true,allowZero: true});
...
//muda a tab padrao do tabviwe
function mudarTab(nome, posicao) {
    var ind = PF(nome).getActiveIndex();
    if (ind !== posicao) {
        PF(nome).select(posicao);
    }
}
...
function onkeydown1(value, dlg, e) {
...
function atualizaTabela(altura) {
    if (altura === undefined) {
        altura = 215;
    }
...
function atualizaTabelaLivro(altura) {
    if (altura === undefined) {
        altura = 540;
    }
    var o = $('.ok').height() - altura;
    $('.ui-datatable-scrollable-body').css('height', o);
}
